...
import { motion } from 'framer-motion';
import Tilt from 'react-parallax-tilt'; // default import ✅
...
const ServiceCard = ({ title, description, imageSrc }: ServiceCardProps) => {
  ...
  return (
    <Tilt
      // 🔁 Mapping from your old options:
      // reverse        -> tiltReverse
      tiltReverse={false}
      // max            -> tiltMaxAngleX / tiltMaxAngleY
      tiltMaxAngleX={25}
      tiltMaxAngleY={25}
      ...
      // transition     -> (no boolean prop; omit)
      // axis           -> tiltAxis ("x" | "y"); omit for both axes
      // reset          -> reset
      reset={true}
      // easing         -> easing (cubic-bezier string supported)
